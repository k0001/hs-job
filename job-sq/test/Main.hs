...
main :: IO ()
main = Di.new \di0 -> A.withAcquire
   (Sq.tempPool (Di.Core.filter (\l _ _ -> l >= Df1.Info) di0))
   \db -> do
      Sq.migrate db "migs" Job.Sq.migrations \_ -> pure ()

      let q = Job.Sq.queue db
